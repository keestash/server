    ...
    public function testRegularCase(): void {
        ...
        $response = $this->getApplication()
            ->handle(
                $this->getRequest(
                    IVerb::DELETE
                    , str_replace(':advid', $additionalData->getId(), ConfigProvider::PASSWORD_MANAGER_CREDENTIAL_ADDITIONAL_DATA_DELETE)
                    , []
                    , $user
                    , $headers
                )
    ...
    public function testNonExisting(): void {
        ...
        $response = $this->getApplication()
            ->handle(
                $this->getRequest(
                    IVerb::DELETE
                    , str_replace(':advid', Uuid::uuid4()->toString(), ConfigProvider::PASSWORD_MANAGER_CREDENTIAL_ADDITIONAL_DATA_DELETE)
                    , []
                    , $user
                    , $headers
                )
