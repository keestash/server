    ...
    public function add(Setting $setting): void
    {
        try {
            ...
            $queryBuilder->insert("`setting`")
                ...
                ->setParameter(0, $setting->getKey())
                ->setParameter(1, $setting->getValue())
                ->setParameter(2, $this->dateTimeService->toYMDHIS($setting->getCreateTs()))
                ->executeStatement();
        } catch (Exception $exception) {
            $this->logger->error('error inserting setting', ['exception' => $exception]);
