...
class Loader implements ILoader {
    ...
    private function isValidInfo(?array $info): bool {
        if (null === $info) {
            $this->logger->info('no info given');
            return false;
        }
    ...
    private function overrideDefaultApp(IApp $app): void {
        ...
        if ($app->getOrder() === $currentApp->getOrder() && $app->getId() !== $currentApp->getId()) {
            throw new DuplicatedSameOrderException("there are two apps with the same order ({$app->getId()} and {$currentApp->getId()})");
        }
...
//        $this->loadJs($app);
    ...
    private function loadTemplate(IApp $app): void {
